...
var $author$project$Calendar$daysRange = function (_v0) {
	var date = _v0.date;
	var year = $justinmimbs$date$Date$year(date);
	var month = $justinmimbs$date$Date$month(date);
	var start = A3($justinmimbs$date$Date$fromCalendarDate, year, month, 1);
	var until = A3($justinmimbs$date$Date$add, $justinmimbs$date$Date$Months, 1, start);
	return A2(
		$elm$core$List$map,
		A2($elm$core$Basics$composeR, $justinmimbs$date$Date$day, $elm$core$String$fromInt),
		A4($justinmimbs$date$Date$range, $justinmimbs$date$Date$Day, 1, start, until));
};
...
var $author$project$Calendar$createDateRanges = function (datetime) {
	return {
		days: $author$project$Calendar$daysRange(datetime),
		hours: $author$project$Calendar$hoursRange,
		minutes: $author$project$Calendar$minutesRange,
		months: $author$project$Calendar$monthsRange,
		seconds: $author$project$Calendar$secondsRange,
		weekdays: $author$project$Calendar$weekdaysRange
	};
};
var $mgold$elm_animation$Animation$A = function (a) {
	return {$: 'A', a: a};
};
var $mgold$elm_animation$Animation$AnimRecord = F7(
	function (start, delay_, dos, ramp, ease_, from_, to_) {
		return {delay_: delay_, dos: dos, ease_: ease_, from_: from_, ramp: ramp, start: start, to_: to_};
	});
...
var $mgold$elm_animation$Animation$static = function (x) {
	return $mgold$elm_animation$Animation$A(
		A7($mgold$elm_animation$Animation$AnimRecord, 0, 0, $mgold$elm_animation$Animation$defaultDuration, $elm$core$Maybe$Nothing, $mgold$elm_animation$Animation$defaultEase, x, x));
};
...
var $author$project$Main$createClockArms = F3(
	function (width, height, datetime) {
		var clockRadius = A2($elm$core$Basics$min, width, height) / 2;
		var calendar = $author$project$Calendar$createDateRanges(datetime);
		var armRadius = clockRadius / 22;
		return _List_fromArray(
			[
				{
				angle: $mgold$elm_animation$Animation$static(0),
				armRadius: armRadius,
				fromTime: $author$project$Calendar$toMonth,
				length: $elm$core$List$length(calendar.months),
				radius: 0.2 * clockRadius,
				range: calendar.months,
				updateRange: $elm$core$Maybe$Nothing
			},
				{
				angle: $mgold$elm_animation$Animation$static(0),
				armRadius: armRadius,
				fromTime: $author$project$Calendar$toWeekday,
				length: $elm$core$List$length(calendar.weekdays),
				...
				angle: $mgold$elm_animation$Animation$static(0),
				armRadius: armRadius,
				fromTime: $author$project$Calendar$toDay,
				length: $elm$core$List$length(calendar.days),
				radius: 0.42 * clockRadius,
				range: calendar.days,
				updateRange: $elm$core$Maybe$Just($author$project$Calendar$daysRange)
			},
				{
				angle: $mgold$elm_animation$Animation$static(0),
				armRadius: armRadius,
				fromTime: $elm$time$Time$toHour,
				length: $elm$core$List$length(calendar.hours),
				...
				angle: $mgold$elm_animation$Animation$static(0),
				armRadius: armRadius,
				fromTime: $elm$time$Time$toMinute,
				length: $elm$core$List$length(calendar.minutes),
				...
				angle: $mgold$elm_animation$Animation$static(0),
				armRadius: armRadius,
				fromTime: $elm$time$Time$toSecond,
				length: $elm$core$List$length(calendar.seconds),
...
var $author$project$Calendar$fromPosix = $justinmimbs$date$Date$fromPosix;
...
var $author$project$Main$init = function (_v0) {
	...
	var timezone = $elm$time$Time$utc;
	var time = $elm$time$Time$millisToPosix(currentTime);
	var date = A2($author$project$Calendar$fromPosix, timezone, time);
	var datetime = {date: date, time: time};
	var clockArms = A3($author$project$Main$createClockArms, width, height, datetime);
	return _Utils_Tuple2(
		{clockArms: clockArms, date: date, delta: 0, height: height, time: time, timezone: timezone, width: width},
	...
	function (x, _v0) {
		...
		return $mgold$elm_animation$Animation$A(
			_Utils_update(
				a,
				{
					dos: $mgold$elm_animation$Animation$Duration(x)
				}));
	});
...
var $mgold$elm_animation$Animation$animate = F2(
	function (clock, _v0) {
		...
		var delay_ = _v0.a.delay_;
		var dos = _v0.a.dos;
		var ramp = _v0.a.ramp;
		var from_ = _v0.a.from_;
		var to_ = _v0.a.to_;
		var ease_ = _v0.a.ease_;
		...
		if (_Utils_eq(newTo, a.to_)) {
			...
			if ($mgold$elm_animation$Animation$isStatic(u)) {
				return $mgold$elm_animation$Animation$A(
					_Utils_update(
						a,
						{ramp: $elm$core$Maybe$Nothing, start: clock, to_: newTo}));
			} else {
				if (A2($mgold$elm_animation$Animation$isScheduled, clock, u)) {
					return $mgold$elm_animation$Animation$A(
						_Utils_update(
							a,
							{ramp: $elm$core$Maybe$Nothing, to_: newTo}));
				} else {
					if (A2($mgold$elm_animation$Animation$isDone, clock, u)) {
						return $mgold$elm_animation$Animation$A(
							_Utils_update(
								a,
								{delay_: 0, from_: a.to_, ramp: $elm$core$Maybe$Nothing, start: clock, to_: newTo}));
					} else {
						...
						var newSpeed = function () {
							...
							if (_v0.$ === 'Speed') {
								...
								return $mgold$elm_animation$Animation$Speed(
									A3($mgold$elm_animation$Animation$spd, a.dos, a.from_, a.to_));
							}
						}();
						return $mgold$elm_animation$Animation$A(
							A7(
	...
	function (msg, model) {
		switch (msg.$) {
			...
			case 'UpdateTime':
				var newTime = msg.a;
				var updateDates = F2(
					function (datetime, clockArm) {
						var _v2 = clockArm.updateRange;
						if (_v2.$ === 'Nothing') {
							return clockArm;
						} else {
							var updateRange = _v2.a;
							var newRange = updateRange(datetime);
							var newLength = $elm$core$List$length(newRange);
							return _Utils_update(
								clockArm,
								{length: newLength, range: newRange});
						}
					});
				var newDate = A2($author$project$Calendar$fromPosix, model.timezone, newTime);
				var newDatetime = {date: newDate, time: newTime};
				var newAnimatedAngle = function (_v1) {
					var angle = _v1.angle;
					var length = _v1.length;
					var fromTime = _v1.fromTime;
					var newAngle = (A2(fromTime, model.timezone, newTime) / length) * 360;
					return A2(
						...
						A2(
							...
							750,
							A3($mgold$elm_animation$Animation$retarget, model.delta, newAngle, angle)));
				};
				var updateAngle = function (clockArm) {
					return _Utils_update(
						clockArm,
						{
							angle: newAnimatedAngle(clockArm)
						});
				};
				var newClockArms = A2(
					$elm$core$List$map,
					A2(
						$elm$core$Basics$composeR,
						updateDates(newDatetime),
						updateAngle),
					model.clockArms);
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{clockArms: newClockArms, date: newDate, time: newTime}),
					$elm$core$Platform$Cmd$none);
			default:
				var newDelta = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{delta: model.delta + newDelta}),
					$elm$core$Platform$Cmd$none);
		}
	});
var $author$project$Color$HSL = F3(
	function (a, b, c) {
		return {$: 'HSL', a: a, b: b, c: c};
	});
...
var $author$project$Draw$drawArc = function (_v0) {
	...
	var innerRadius = _v0.innerRadius;
	var outerRadius = _v0.outerRadius;
	var cornerRadius = _v0.cornerRadius;
					...
					_List_fromArray(
						[
							...
							$author$project$Draw$drawArc(
								{cornerRadius: armRadius, endAngle: newAngle + (armRadius / radius), innerRadius: radius - armRadius, outerRadius: radius + armRadius, startAngle: (-armRadius) / radius}) + (A3($author$project$Draw$drawDot, radius, newAngle - (armRadius / radius), 0.8 * armRadius) + 'Z')),
...
var $author$project$Draw$drawTicks_ = function (_v0) {
	...
	var positionTicksOnArc = F2(
		function (index, tick) {
			...
			var cx = _v2.a;
			var cy = _v2.b;
			return {cx: cx, cy: cy, tick: tick};
		});
	var newRange = A2($elm$core$List$indexedMap, positionTicksOnArc, range);
					...
					return A2(
						...
						function (currentTime) {
							return $elm$json$Json$Decode$succeed(
								{currentTime: currentTime, height: height, width: width});
						},
						A2($elm$json$Json$Decode$field, 'currentTime', $elm$json$Json$Decode$int));
				},
				A2($elm$json$Json$Decode$field, 'height', $elm$json$Json$Decode$int));
		},
		A2($elm$json$Json$Decode$field, 'width', $elm$json$Json$Decode$int)))({"versions":{"elm":"0.19.1"},"types":{"message":"Main.Msg","aliases":{"Time.Era":{"args":[],"type":"{ start : Basics.Int, offset : Basics.Int }"}},"unions":{"Main.Msg":{"args":[],"tags":{"UpdateTime":["Time.Posix"],"UpdateTimeZone":["Time.Zone"],"Animate":["Basics.Float"]}},"Basics.Float":{"args":[],"tags":{"Float":[]}},"Time.Posix":{"args":[],"tags":{"Posix":["Basics.Int"]}},"Time.Zone":{"args":[],"tags":{"Zone":["Basics.Int","List.List Time.Era"]}},"Basics.Int":{"args":[],"tags":{"Int":[]}},"List.List":{"args":["a"],"tags":{}}}}})}});
